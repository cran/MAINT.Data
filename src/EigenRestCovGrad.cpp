	...
	NumericVector::iterator xpos,const bool FixedArrays,RCTMATTP& Jacob)
{
	...
	static VectorXi nullrows,validcols;
	...
	if (!FixedArrays) {
		...
		if (SigmaSrU.rows()!=p || SigmaSrU.cols()!=p) SigmaSrU.resize(p,p);
		if (fullpar.size()!=nvcovpar)  fullpar.resize(nvcovpar);
	}
	Sigma = RestCov<SQMATTP>(q,xpos,2,FixedArrays);
	LLT<SQMATTP> SigmaSr(Sigma.llt());    
	SigmaSrU = SigmaSr.matrixU();      
	...
	SetZero<RCTMATTP>(JacobC1,nvcovpar,nvcovpar,true); 
	...
	for (int r=q-2;r>=0;r--)  {
		for (int c=q;c<p;c++) for (int i=r+1;i<q;i++) if (c!=q+i)  {
			cind = p+utind0(r,c);
			for (int r1=0;r1<nvcovpar;r1++) JacobC1(r1,cind) -= 
				*(xpos+p+utind0(r,i)) * JacobC1(r1,p+utind0(i,c))  / *(xpos+i); 
		}  
		for (int c=r+1;c<q;c++) for (int j=q;j<p;j++) if (j!=q+c)  {
			cind = p+utind0(r,c);
			for (int r1=0;r1<nvcovpar;r1++) JacobC1(r1,cind) -= 
				 *(xpos+p+utind0(r,j)) * JacobC1(r1,p+utind0(c,j))  / *(xpos+c); 
		}
	}  
	for (int c=q-1;c>0;c--) for (int j=q;j<p;j++) if (j!=q+c)  {
		Asum = 0.;
		for (int c0=0;c0<c;c0++) Asum += 
			*(xpos+p+utind0(c0,c)) * *(xpos+p+utind0(c0,j)) ; 
	...
	NumericVector::iterator xpos,const bool FixedArrays,RCTMATTP& Jacob)
{
	...
	if (!FixedArrays && ppar.size()!=npcovpar)  ppar.resize(npcovpar);
	NumericVector pparasNV(wrap(ppar));
	SetZero<RCTMATTP>(JacobC1,npcovpar,npcovpar,true); 
